...
class Async1000mileTask:
	...
	def requesUrl(self):
		...
		url_list = [self.SHOP_URL+i.get('href') for i in list_lxml.select('#contents > div > ul > li > div.thumbnail > a')]
		img_list = ['http:'+i.get('src') for i in list_lxml.select('#contents > div > ul > li > div.thumbnail > a > img')]
		name_list = [i.find_all(text=True)[0] for i in list_lxml.select('#contents > div > ul > li > div.information > p.name > a')]
