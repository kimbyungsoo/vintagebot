...
class AsyncCemeteryparkTask:
	def __init__(self):
		self.SHOP_URL = "http://m.cemeterypark.kr"
		self.headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/71.0.3578.98 Safari/537.36'}
		self.goods = []
	def requesUrl(self):
		list_html = requests.get(url=self.SHOP_URL + "/product/list.html?cate_no=24", headers=self.headers).content
		...
		name_list = [i.find_all(text=True)[0] for i in list_lxml.select('div.information > div.name > a')]
	...
	def saveDB(self):
		if len(self.goods) == 0:
			...
			database_filename = './cemeterypark/cemeterypark.db'
...
def main():
	print ('cemeterypark bot start')
	BT = AsyncCemeteryparkTask()
	BT.requesUrl()
