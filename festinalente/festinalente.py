...
class AsyncFestinalenteTask:
	def __init__(self):
		self.SHOP_URL = "http://www.festinalente.kr"
	...
	def requesUrl(self):
		...
		url_list = [self.SHOP_URL+i.get('href') for i in list_lxml.select('a.name')]
		img_list = ['http:'+i.get('src') for i in list_lxml.select('a.prdImg > img')]
		name_list = [i.find_all(text=True)[0] for i in list_lxml.select('a.name')]
	...
	def saveDB(self):
		if len(self.goods) == 0:
			...
			database_filename = './festinalente/festinalente.db'
